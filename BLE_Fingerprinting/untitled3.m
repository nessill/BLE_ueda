...
for packetIdx = 1:numPackets
    ...
    I_offset=round(2*rand,1)-1;
    Q_offset=round(2*rand,1)-1;
    % I/Q不均衡パラメータの定義
    amp_imbalance = 0;
    phase_imbalance = 0;
    % I/Q不均衡を加える
    re_waveform_FIQ = real(waveform) * cos(amp_imbalance/2)+imag(waveform)*sin(amp_imbalance/2)+I_offset;
    im_waveform_FIQ = imag(waveform)*cos(amp_imbalance/2) + real(waveform)*sin(amp_imbalance/2)+Q_offset;
    waveform_FIQ=re_waveform_FIQ+1j * (im_waveform_FIQ);
...
I3 = real(waveform_FIQIQ);
Q3 = imag(waveform_FIQIQ);
